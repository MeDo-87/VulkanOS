...
void MemoryManager::init(multiboot_info *mBoot)
{
    ...
    Manager.MaxMemoryBlocks = (Manager.TotalPhysicalMemory * 1024) / PMMNGR_BLOCK_SIZE;

    memory_map_t *mmap = (memory_map_t *)mBoot->mmap_addr;

    //Mark everything as used
	//! By default, all of memory is in use
	memset (Manager.MemoryBitMap, 0xff,  Manager.MaxMemoryBlocks/ PMMNGR_BLOCKS_PER_BYTE );

    GConsole.WriteHex((UInt32)Manager.MemoryBitMap);
    GConsole << "Max blocks " << Manager.MaxMemoryBlocks;
...
GConsole << "\n";
GConsole.WriteHex(((UInt32)end + Manager.MaxMemoryBlocks/PMMNGR_BLOCKS_PER_BYTE)/PMMNGR_BLOCK_SIZE);
    //Mark the block up to the kernal end + map to be used
    Manager.ReserveMemory(0x00, ((UInt32)end + Manager.MaxMemoryBlocks/PMMNGR_BLOCKS_PER_BYTE)/PMMNGR_BLOCK_SIZE);
    DebugBreak();
    //Create the Map here
...
{
    //Calculate the number of blocks we need for this region, and the alignment in our map
